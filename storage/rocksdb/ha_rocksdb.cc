...
static bool rocksdb_use_aws = false;
...
static MYSQL_SYSVAR_BOOL(
    concurrent_prepare,
    *static_cast<bool *>(&rocksdb_db_options->two_write_queues),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    manual_wal_flush,
    *static_cast<bool *>(&rocksdb_db_options->manual_wal_flush),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    ...
    *static_cast<bool *>(&rocksdb_db_options->allow_concurrent_memtable_write),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    use_direct_reads,
    *static_cast<bool *>(&rocksdb_db_options->use_direct_reads),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    allow_mmap_reads,
    *static_cast<bool *>(&rocksdb_db_options->allow_mmap_reads),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    allow_mmap_writes,
    *static_cast<bool *>(&rocksdb_db_options->allow_mmap_writes),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    is_fd_close_on_exec,
    *static_cast<bool *>(&rocksdb_db_options->is_fd_close_on_exec),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    ...
    *static_cast<bool *>(&rocksdb_db_options->advise_random_on_open),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    use_adaptive_mutex,
    *static_cast<bool *>(&rocksdb_db_options->use_adaptive_mutex),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    ...
    *static_cast<bool *>(&rocksdb_db_options->enable_thread_tracking),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    whole_key_filtering,
    *static_cast<bool *>(&rocksdb_tbl_options->whole_key_filtering),
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_BOOL(
    use_aws, rocksdb_use_aws, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
    "Whether to use aws s3 cloud storage for rocksdb ",
    nullptr, nullptr, false);

static MYSQL_SYSVAR_STR(
    aws_access_key_id, rocksdb_aws_access_key_id,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
    "Aws access key id for aws s3 cloud storage",
    nullptr, nullptr, "");

static MYSQL_SYSVAR_STR(
    aws_secret_access_key, rocksdb_aws_secret_access_key,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
    "Aws secret access key for aws s3 cloud storage",
    nullptr, nullptr, "");

static MYSQL_SYSVAR_STR(
    aws_s3_bucket_name, rocksdb_aws_s3_bucket_name,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static std::string rdb_xid_to_string(const XID &src) {
  ...
  DBUG_ASSERT(src.get_gtrid_length() <= MAXGTRIDSIZE);
  DBUG_ASSERT(src.get_bqual_length() >= 0);
  DBUG_ASSERT(src.get_bqual_length() <= MAXBQUALSIZE);
...
static void rdb_xid_from_string(const std::string &src, XID *const dst) {
  ...
  DBUG_ASSERT(dst->get_gtrid_length() <= MAXGTRIDSIZE);
  DBUG_ASSERT(dst->get_bqual_length() >= 0);
  DBUG_ASSERT(dst->get_bqual_length() <= MAXBQUALSIZE);
...
static bool rocksdb_collect_hton_log_info(handlerton *const /* unused */,
  ...
  for (const auto &wal : live_wal_files) {
    ...
    if (!ret_val) json_wal.add_clone("path_name", &json_path_name);
    if (!ret_val) json_wal.add_clone("size_file_bytes", &json_size_file_bytes);
    if (!ret_val) json_wal_files.insert_clone(index, &json_wal);
    if (ret_val) break;
    ++index;
  }
...
//------------------------
  if(rocksdb_use_aws) {
    ...
    if (!cenv_status.ok()) {
      rdb_log_status_error(cenv_status, "Error creating AwsEnv");
      DBUG_RETURN(HA_EXIT_FAILURE);
    }
    ...
    if(!cenv_status.ok()) {
      rdb_log_status_error(cenv_status, "Error Preparing open DBCloud");
      DBUG_RETURN(HA_EXIT_FAILURE);
    }
  }
//------------------------
  ...
  if (rocksdb_db_options->use_direct_reads ||
    ...
    if (!check_status.ok()) {
      LogPluginErrMsg(ERROR_LEVEL, 0,
                      "Unable to use direct io in rocksdb-datadir: (%s)",
...
// ----------------------
  if(rocksdb_use_aws) {
    ...
    if (!status.ok()) {
      rdb_log_status_error(status, "Error WrapStackableDB");
      DBUG_RETURN(HA_EXIT_FAILURE);
    }
  } else {
...
const std::string ha_rocksdb::generate_cf_name(
  ...
  *per_part_match_found = false;
  ...
  if (!cf_name.empty()) rdb_trim_whitespace_from_edges(cf_name);
...
static ulonglong io_stall_prop_value(
  ...
  if (iter != props.end()) {
    ...
    DBUG_PRINT("warning",
               ("RocksDB GetMapProperty hasn't returned key=%s", key.c_str()));
    DBUG_ASSERT(0);
    return 0;
  }
}
  ...
  for (;;) {
    ...
    if (rocksdb_debug_manual_compaction_delay > 0) {
      ...
      for (uint32_t sleeps = 0; sleeps < rocksdb_debug_manual_compaction_delay;
           sleeps++) {
        RDB_MUTEX_LOCK_CHECK(m_signal_mutex);
        const bool local_stop = m_killed;
        RDB_MUTEX_UNLOCK_CHECK(m_signal_mutex);
        if (local_stop) break;
        my_sleep(1000000);
      }
  ...
  if (status.IsIOError()) {
    switch (err_type) {
      ...
      case RDB_IO_ERROR_BG_THREAD: {
        ...
        /* NO_LINT_DEBUG */
        LogPluginErrMsg(ERROR_LEVEL, 0, "aborting on BG write error.");
        abort();
        break;
      }
